...
We parameterize the amplitudes natively in the Cartesian $(b_L, b_T)$ lattice space to resolve kinematic degeneracies, then map the resulting functions into the continuous Lorentz-invariant $(P \cdot b, b^2)$ phase space. We begin by testing a simple two-dimensional Gaussian profile.
...
The Gaussian parameterization maps easily into the $(P \cdot b, b^2)$ phase space, but it does not satisfy the physical constraints necessary to extract the Sivers shift.
...
Figures \ref{fig:A2B_amplitudes_fit_P1} and \ref{fig:combined_fit_P1} show the results of the correlated minimization. The parameterization (represented by the continuous surfaces and shaded error bands) aligns with the discrete lattice data across the Cartesian $(b_L/a, b_T/a)$ grid and the Lorentz-invariant $(P \cdot b, b^2)$ phase space. Excluding the short-distance data ($|\vect{b}| < 3a$) ensures the fit captures the long-range dynamics without skewing from ultraviolet lattice artifacts.
