...
The fitting and stability analysis in this chapter follows a sequential progression towards finding the final model. In Section~\ref{sec:gaussian_transformation}, we demonstrate why the simple Gaussian ansatz fails due to non-integrable longitudinal growth ($\alpha < \beta$). In Section~\ref{sec:discretization_effects}, we isolate ultraviolet lattice discretization artifacts by applying a spatial cutoff $|\vect{b}| \ge 3a$. In Section~\ref{sec:pysr_methodology}, using genetic programming with an asymptotic decay penalty, we discover the generalized Lorentzian power-law tail $(1+b^2)^{-j}$ and the exponential area-law factor $e^{-f\eta}$. In Section~\ref{sec:factorization_test}, we test directly on the lattice data whether the amplitudes factorize as a product of independent $(P\cdot b)$ and $b^2$ profiles. In Section~\ref{sec:systematic_exploration} and Section~\ref{sec:candidate_selection}, we determine the coupled Lorentzian model's parameter behavior across individual staple extents $\eta \in [6, 10]$, select the asymptotic model $c(\eta) = c(1 - k_1/\eta - k_2/\eta^2)$ based on AIC, and enforce physical anti-quark suppression at large negative $x$ ($x < -0.2$). In Section~\ref{sec:simultaneous_fit_strategy}, we perform a correlated Jackknife $\chi^2$ minimization across all amplitudes, cancel the shared $e^{-f\eta}$ factor, and extract the $x$-dependent Sivers shift at each of our four simulated nucleon momenta. Finally, in Section~\ref{sec:rejection_exponential_longitudinal}, we use the completed global fit as a benchmark to confirm that explicit factorized models fail on $\tAmp_{2B}^{\text{Im}}$ and $\tAmp_{12B}^{\text{Re}}$, requiring instead the coupled denominator $[1 + c(P\cdot b)^2/P_L^2 + d b^2]^j$.
...
\end{align}
To transform this into the invariant variable space, we substitute $b_T^2 = b^2 - b_L^2$ and apply the Lorentz-invariant projection $b_L = (P \cdot b) / (-P_L)$ derived in Section \ref{sec:lorentz_mapping}. Squaring this substitution gives
\begin{align}
...
This means the amplitude decays faster in the transverse direction than in the longitudinal direction, leading to a non-integrable expansion in the $(P \cdot b, b^2)$ space; Fig.~\ref{fig:gaussian_failure} shows the resulting poor fit quality. A symmetric 2D Gaussian decay profile is too rigid to describe the non-local operator.
...
Unlike traditional regression, which optimizes the parameters of a pre-determined function, symbolic regression searches the space of mathematical expressions and their parameters simultaneously. We use \texttt{PySR} \cite{Cranmer2023pysr}, a framework that uses Genetic Programming to discover interpretable equations from data by balancing accuracy and simplicity.
...
The power-law tail $(1 + b^2)^{-j}$ with $j > 1/2$ decays algebraically at large separations, more slowly than a Gaussian but still absolutely integrable over the real line. Unlike the Gaussian model rejected in Section~\ref{sec:gaussian_transformation}, the power-law denominator guarantees convergent Fourier integrals for any positive exponent $j > 1/2$. The factorized structure $e^{-f\eta} \times g(b^2)$ also allows the shared $\eta$-dependent exponential to cancel analytically in the Sivers shift ratio, which we use in Section~\ref{sec:simultaneous_fit_strategy}.
...
The straight-line diagnostic in Fig.~\ref{fig:gaussian_straightline_test} confirms this directly: the lattice data curve upward at large $b^2$ rather than following the Gaussian's expected straight line, consistent with the generalized Lorentzian power-law tail discovered by the genetic algorithm.
...
\subsection{Test for Factorization in $(P\cdot b)$ and $b^2$} \label{sec:factorization_test}
...
Based on the $\chi^2/\text{dof}$ minimizations and sequential fitting at individual $\eta$ slices, we determined the structural parameterizations for the three surviving invariant amplitudes ($\tAmp_{2B}^{\text{Re}}$, $\tAmp_{2B}^{\text{Im}}$, and $\tAmp_{12B}^{\text{Re}}$; $\tAmp_{12B}^{\text{Im}}$ remains excluded, as noted above).
...
We then decompose the complex effective amplitudes into their real (T-even) and imaginary (T-odd) components, dropping $\tAmp_{12B}^{\text{Im}}$ from the numerator as noted above:
\begin{equation}
...
\end{equation}
Substituting the analytical models from Eq. (\ref{eq:model_ReA2B})-(\ref{eq:model_ImA2B}), the shared $e^{-f\eta}$ term is independent of the integration variable $b\cdot P$ and cancels between the numerator and denominator integrals. Applying the Lorentz-invariant projection $b_L=\frac{P\cdot b}{-P_L}$ and $b_T^2=b^2-b_L^2$, we arrive at the final integrands in the invariant frame:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
To enforce this shared $\eta$-dependence across the amplitudes, we use a simultaneous global fit. Forcing the parameter $f$ to be identical across all three amplitude models ($\tAmp_{2B}^{\text{Re}}$, $\tAmp_{2B}^{\text{Im}}$, and $\tAmp_{12B}^{\text{Re}}$) ensures theoretical consistency and stabilizes the parameter extraction.
...
To resolve this, we perform a correlated $\chi^2$ minimization using the full covariance matrix. This accounts for the cross-correlations inherent in the lattice data, both across different quark spatial separations and across the real and imaginary amplitude components. 
...
The extracted parameters from the simultaneous fit for the lowest simulated nucleon momentum, $P_1 = -1$ ($\hat{\zeta} \approx 0.091$), are summarized below.
...
\begin{figure}[htbp]
    ...
    \caption{2D representation of the full Jackknife covariance matrix used in the simultaneous fit for $P_1=-1$. The prominent off-diagonal bands show the strong correlations across the parameter space.}
...
\pagebreak
\subsubsection{Rejection of Separable Longitudinal and Transverse Factorization}
\label{sec:rejection_exponential_longitudinal}
Having completed the global simultaneous fit above, we return to the separability hypothesis discussed in Section \ref{sec:factorization_test} and test it directly. We constructed candidate models that multiply an independent longitudinal profile by various transverse $b^2$ profiles, including combinations of Gaussian, Lorentzian power-law, and exponential-decay forms, and evaluated each with full Jackknife fits.

The fit results are summarized in Fig.~\ref{fig:rejection_gaussian_transverse} (see its caption for the poor $\chi^2/\text{dof}$ when exponential decay in $(P\cdot b)$ is paired with a Gaussian transverse profile). Even the better-performing combinations give $\chi^2/\text{dof}\approx 2$, with the coefficient $c$ governing $(P\cdot b)$ evaluating to a negative value consistent with zero within Jackknife uncertainties, which is unstable and divergent under a Fourier transform.
...
In the Lorentz-invariant variable space $(P\cdot b, b^2)$, the longitudinal separation $b_L = (P\cdot b)/(-P_L)$ and transverse separation $b_T = \sqrt{b^2 - b_L^2}$ are geometrically coupled. Factorized models force an artificial independence between $P\cdot b$ and $b^2$. In contrast, coupling $(P\cdot b)^2$ and $b^2$ within a single denominator,
    ...
    \end{equation}
preserves smooth analyticity at $P\cdot b = 0$, satisfies power-law integrability, and yields stable, positive parameters across all three amplitudes, matching the values already obtained from the global fit above (e.g., $c_{\text{im}} = 0.2253(69)$, $c_{\text{reA12B}} = 0.0773(46)$, $a_{\text{im}} = +0.267(13)$). This confirms that the coupled Lorentzian structure adopted for the global fit is indeed preferred over any explicit factorization of $(P\cdot b)$ and $b^2$.

\renewcommand{\PLval}{-2}
\subsubsection{\texorpdfstring{Simultaneous Fit Results for $\vect{P}\cdot a\hat{L}/(2\pi)=\{\PLval,0,0\}$}{TEXT}}
Figs.~\ref{fig:A2B_amplitudes_fit_P2}, \ref{fig:combined_fit_P2}, \ref{fig:cov_matrix_2d_P2}, and \ref{fig:cov_matrix_3d_P2} show the corresponding fit and covariance matrix at this momentum.
\input{SimultaneousFitsCov/Fit_Table_PL\PLval_bmin3}
\begin{figure}[htbp]
    \centering
    \begin{tabular}{ccc}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{SimultaneousFitsCov/SimulFitCovMatrix-A12B-A2B-FT-a_im-a_re-a_reA12B-c_im-c_re-c_reA12B-d_im-d_re-d_reA12B-f-j_im-j_re-j_reA12B-k1_re-k2_re__bmin3_eta610_PL\PLval.pdf}
    \caption{2D representation of the full Jackknife covariance matrix used in the simultaneous fit for $P_L=-2$.}
...
\subsubsection{\texorpdfstring{Simultaneous Fit Results for $\vect{P}\cdot a\hat{L}/(2\pi)=\{\PLval,0,0\}$}{TEXT}}
Figs.~\ref{fig:A2B_amplitudes_fit_P3}, \ref{fig:combined_fit_P3}, \ref{fig:cov_matrix_2d_P3}, and \ref{fig:cov_matrix_3d_P3} show the corresponding fit and covariance matrix at this momentum.
\input{SimultaneousFitsCov/Fit_Table_PL\PLval_bmin3}
\begin{figure}[htbp]
    \centering
    \begin{tabular}{ccc}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{SimultaneousFitsCov/SimulFitCovMatrix-A12B-A2B-FT-a_im-a_re-a_reA12B-c_im-c_re-c_reA12B-d_im-d_re-d_reA12B-f-j_im-j_re-j_reA12B-k1_re-k2_re__bmin3_eta610_PL\PLval.pdf}
    \caption{2D representation of the full Jackknife covariance matrix used in the simultaneous fit for $P_L=-3$.}
...
\subsubsection{\texorpdfstring{Simultaneous Fit Results for $\vect{P}\cdot a\hat{L}/(2\pi)=\{\PLval,0,0\}$}{TEXT}}
Figs.~\ref{fig:A2B_amplitudes_fit_P4}, \ref{fig:combined_fit_P4}, \ref{fig:cov_matrix_2d_P4}, and \ref{fig:cov_matrix_3d_P4} show the corresponding fit and covariance matrix at this momentum.
\input{SimultaneousFitsCov/Fit_Table_PL\PLval_bmin3}
\begin{figure}[htbp]
    \centering
    \begin{tabular}{ccc}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{SimultaneousFitsCov/SimulFitCovMatrix-A12B-A2B-FT-a_im-a_re-a_reA12B-c_im-c_re-c_reA12B-d_im-d_re-d_reA12B-f-j_im-j_re-j_reA12B-k1_re-k2_re__bmin3_eta610_PL\PLval.pdf}
    \caption{2D representation of the full Jackknife covariance matrix used in the simultaneous fit for $P_L=-4$.}
