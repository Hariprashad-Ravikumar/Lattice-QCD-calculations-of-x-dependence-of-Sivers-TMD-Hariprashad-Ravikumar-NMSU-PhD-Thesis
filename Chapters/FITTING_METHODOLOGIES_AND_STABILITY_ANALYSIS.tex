...
Extracting the momentum-fraction $x$-dependence of transverse momentum-dependent parton distributions (TMDs) from lattice QCD requires reconstructing continuous light-cone correlation functions from discrete Euclidean correlation functions. Our lattice calculations evaluate non-local quark bilinear operators connected by staple-shaped Wilson lines of length $\eta$, simulated across a discrete spatial grid $(b_L, b_T)$ and discrete nucleon boost momenta $P_L$. 

The lattice data exhibits uneven statistical precision across the invariant amplitudes. The real unpolarized amplitude, $\tAmp_{2B}^{\text{Re}}$, has small statistical errors across the entire spatial volume. In contrast, the imaginary unpolarized amplitude $\tAmp_{2B}^{\text{Im}}$ and the real Sivers amplitude $\tAmp_{12B}^{\text{Re}}$ have noticeably larger gauge noise. The imaginary Sivers amplitude $\tAmp_{12B}^{\text{Im}}$ is consistent with zero within statistical uncertainties and is dropped from further numerical fitting. We therefore establish our spatial parameterization strategy primarily on $\tAmp_{2B}^{\text{Re}}$, and then extend the discovered structure to $\tAmp_{2B}^{\text{Im}}$ and $\tAmp_{12B}^{\text{Re}}$ in a joint simultaneous fit.

We fit the amplitudes directly in Cartesian coordinates $(b_L, b_T)$ rather than in Lorentz-invariant variables $(P \cdot b, b^2)$. Because $b^2 = b_L^2 + b_T^2 \ge 0$, different physical configurations—such as long longitudinal separation with small transverse separation versus short longitudinal separation with large transverse separation—map onto the identical value of $b^2$. Fitting directly in the invariant variable space compresses these orthogonal directions, causing numerical instabilities. Parameterizing the 2D surface over $(b_L, b_T)$ separates the longitudinal boost profile from the transverse decay before projecting into the continuous Lorentz frame to evaluate the Fourier integrals.

\subsection{Methodological Roadmap}
\label{sec:chapter_roadmap}

To establish a physically robust and stable parameterization of the lattice amplitudes, our analysis follows a systematic seven-stage workflow, summarized in Table~\ref{tab:methodology_roadmap}.

\begin{table}[htbp]
\centering
\renewcommand{\arraystretch}{1.3}
\caption{Systematic workflow for extracting $x$-dependent TMD distributions and the Sivers shift from lattice correlation functions.}
\label{tab:methodology_roadmap}
\scalebox{0.82}{
\begin{tabular}{|c|l|p{7.2cm}|p{5.5cm}|}
\hline
\textbf{Stage} & \textbf{Analysis Step} & \textbf{Objective \& Methodology} & \textbf{Key Finding / Decision} \\
\hline
1 & \textbf{UV Filtering} & Exclude data with $|\vect{b}| < 3a$ to eliminate short-distance lattice artifacts and hypercubic rotational symmetry breaking. & Restricts fits to non-perturbative long-range dynamics ($|\vect{b}| \ge 3a$). \\
\hline
2 & \textbf{Gaussian Baseline} & Test simple 2D Gaussian ansatz $\gamma e^{-\alpha b_L^2 - \beta b_T^2}$ in Cartesian space. & Fails: yields $\alpha < \beta$, producing a divergent Fourier transform and poor $\chi^2/\text{dof} \approx 235$. \\
\hline
3 & \textbf{Symbolic Regression} & Run unbiased genetic programming (\texttt{PySR}) with an asymptotic decay penalty on $\tAmp_{2B}^{\text{Re}}$. & Discovers generalized Lorentzian power-law tails $(1+b^2)^{-j}$ and area-law factor $e^{-f\eta}$. \\
\hline
4 & \textbf{Separability Tests} & Test factorized models ($(P\cdot b\text{-part}) \times (b\text{-part})$) across all three surviving amplitudes. & Fails for $\tAmp_{2B}^{\text{Im}}$ and $\tAmp_{12B}^{\text{Re}}$: forces non-separable coupled denominator $[1 + c(P\cdot b)^2/P_L^2 + d b^2]^j$. \\
\hline
5 & \textbf{Sequential $\eta$-Slices} & Fit individual staple extents $\eta \in [6, 10]$ at fixed powers to isolate parameter evolution. & Parameter $d$ is independent of $\eta$; $\eta$-variation is isolated strictly to $c(\eta)$ for $\tAmp_{2B}^{\text{Re}}$. \\
\hline
6 & \textbf{Asymptotic Selection} & Evaluate candidate $c(\eta)$ models using AIC and physical light-cone boundary conditions. & Model $c(\eta) = c(1 - k_1/\eta - k_2/\eta^2)$ selected for suppressing anti-quarks at large $|x|$ ($x < -0.2$). \\
\hline
7 & \textbf{Global Simultaneous Fit} & Correlated $\chi^2$ minimization with full Jackknife covariance matrix across all amplitudes and momenta. & Cancels common $e^{-f\eta}$ factor; yields stable $x$-dependent Sivers shift $\langle k_y \rangle_{TU}(x)$. \\
\hline
\end{tabular}
}
\end{table}

The following sections detail each step of this progression.
...
\centering
\renewcommand{\arraystretch}{1.3}
\caption{Comparison of the four factorized longitudinal-transverse models across nucleon boost momenta $P_L$, with extracted parameters shown at $P_L = -1$. While models with power-law transverse envelopes describe $\tAmp_{2B}^{\text{Re}}$ passably, all four factorized ans\"{a}tze fail on $\tAmp_{2B}^{\text{Im}}$ and $\tAmp_{12B}^{\text{Re}}$.}
...
\begin{enumerate}
    ...
    \item \textbf{Physical requirement of relativistic coupling}: Relativistic hadrons do not exhibit factorized $(x, k_T)$ distributions. A Lorentz boost shifts both longitudinal and transverse components of the non-local quark-gluon correlator simultaneously. Coupling $(P\cdot b)^2$ and $b^2$ within a single denominator,
    \begin{equation}
